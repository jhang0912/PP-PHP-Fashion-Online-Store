...
$test = new getMSC($_GET['mainSortClicked'], $_GET['ndSortClicked'],$_GET['text']);
$reference = ($_GET['mainSortClicked'] == 'shoes' || $_GET['ndSortClicked'] == 'shoes') ? 'shoes' : 'collection';
?>
  <?php
  foreach ($test->call_MSC() as $key => $value) {
  ?>
    <div class="spec-card-<?=$_GET['mainSortClicked']?> col-4 flex-wrap justify-content-center align-items-center border overflow-hidden bg-light" data-index="<?= $key ?>">
